...
export const MapPanel = ({ userCoords = null, activeRoute = null }) => {
  ...
  // Load live vehicle and incident data from the Node backend.
  useEffect(() => {
    const load = async () => {
      try {
        const [vehicles, incidentData] = await Promise.all([
          api.getVehicles(),
          api.getIncidents(),
        ]);
        setLiveVehicles(vehicles.filter(v => v.position));
        setIncidents(incidentData.filter(i => i.position));
      } catch (error) {
        ...
        <Marker key={v.id} position={v.position} icon={vehicleIcon}>
          <Popup>
            <div style={{ padding: '4px' }}>
              ...
              <Badge type={v.status === 'IN_TRANSIT' ? 'success' : 'warning'}>{v.status}</Badge>
            </div>
          </Popup>
        </Marker>
      ))}
