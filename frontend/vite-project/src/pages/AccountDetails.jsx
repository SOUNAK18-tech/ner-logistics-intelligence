...
export const AccountDetails = () => {
  ...
  const handleChangePw = (e) => {
    ...
    toast('Password change requires the backend password endpoint.');
  };

  const handleRemoveAccount = () => {
    setDeleteDialog(false);
    toast('Account removal requires the backend account-removal endpoint.', {
      icon: '⏳',
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{ color: 'var(--slate)', padding: '24px 0' }}>Loading account details...</div>
      ) : (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 640 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 24px' }}>
            {[
              { label: 'System User ID', value: user?.userId ?? '—', mono: true },
              { label: 'Role',           value: user?.role ?? '—' },
              { label: 'Email',          value: user?.email ?? '—' },
              { label: 'Account Status', value: accountStatus,
                pill: accountStatus === 'APPROVED'
